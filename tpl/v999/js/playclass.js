...
function PlayHistoryClass() {
				...
				if(result['rcode']>-1){
					...
					if(result['r']!=null&&result['r'].length>0){
						...
						for(var i in result['r']){
							...
							innerStr += "<li><h5><a href=\"" + data['vod_readurl'] + "\">"+data['vod_name']+"</a><em>/</em><a href='"+data['vod_palyurl']+"' target='_blank'>"+data['url_name']+"</a></h5><label><a class=\"color\" href=\"" + data['vod_palyurl'] + "\">继续观看</a></label>";
							if(data['vod_playnext']){
							innerStr +="<label><a class=\"color\" href=\"" + data['vod_playnext'] + "\">下一集</a></label>";
							}
							innerStr +="<a href=\"javascript:;\" target='_blank' class='delck' data=\""+data['id']+"\" mtype='ajax'></a></li>" ;
						}
	...
	this.removeHistory = function(ii, type) {
		...
		if (type == 'inck') {
			...
			if (cookieStr.indexOf("zanpian_vod_v=") != -1 || cookieStr.indexOf("_$_|") != -1) {
				var newCookieStr = "";
				...
				document.cookie = "zanpian_vod_v=" + newCookieStr + timeAndPathStr;
				return true;
			}
			return false;
		}else{
...
function _GC() {
	...
	document.cookie = "zanpian_vod_v=; path=/";
}
